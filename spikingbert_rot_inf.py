...
"""PyTorch BERT model with per-layer H2/H3 rotations fused into linear weights."""
...
class BertLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        layer_output = self.feed_forward_chunk(attention_output, H1_sign=H1_sign)
        outputs = (layer_output,) + outputs

        return outputs

    def feed_forward_chunk(self, attention_output, H1_sign=None):
        intermediate_output = self.intermediate(attention_output, H1_sign=H1_sign)
        layer_output = self.output(intermediate_output, attention_output, H1_sign=H1_sign)
        return layer_output
