...
- `snn_rot_new3_110M/`: pretrained checkpoint directory.
- `snn_rot_new3_110M_fused/`: fused inference checkpoint directory generated by `convertor.py`.
...
```bash
python convertor.py \
  --input snn_rot_new3_110M/step_550000 \
  --output snn_rot_new3_110M_fused/step_550000 \
...
```text
Saved fused weights to snn_rot_new3_110M_fused/step_550000/model.safetensors
Saved fusion report to snn_rot_new3_110M_fused/step_550000/fusion_report.json
Removed 24 keys
Kept bert.H1: True
...
model = BertForMaskedLM.from_pretrained(
    "snn_rot_new3_110M_fused/step_550000",
    config=config,
...
The pretraining script has two different data stages. The raw dataset loader is still present in `spiking_pretrain_rot.py`, but the expensive tokenization/chunking stage is currently commented out. The run that produced `snn_rot_new3_110M` trained from a precomputed Hugging Face `DatasetDict` cache:
...
The original run that produced `snn_rot_new3_110M` used:
...
python finetune_spiking_rot.py \
  --task_name sst2 \
  --model_name_or_path snn_rot_new3_110M/step_550000 \
